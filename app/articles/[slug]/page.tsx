...
async function getArticleData(slug: string) {
    if (!slug) return null;
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/public/articles/${slug}`, { cache: 'no-store' });
...
export default async function ArticlePage({ params, searchParams }: { params: Promise<{ slug: string }>, searchParams?: Promise<{ headerColor?: string, paragraphColor?: string, textColor?: string, fontFamily?: string, sidebarPosition?: string }> }) {
    ...
    if (resolvedSearchParams?.headerColor) {
        ...
        try {
            const res = await fetch(process.env.NEXT_PUBLIC_API_URL ? `${process.env.NEXT_PUBLIC_API_URL}/api/public/article-settings` : "${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/public/article-settings", { cache: 'no-store' });
            if (res.ok) {
