...
async function getArticleData(slug: string) {
    if (!slug) return null;
    try {
        const res = await fetch(`http://localhost:5000/api/public/articles/${slug}`, { cache: 'no-store' });
...
export default async function ArticlePage({ params, searchParams }: { params: Promise<{ slug: string }>, searchParams?: { layout?: string } }) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (appearance && appearance.articleLayout && appearance.articleLayout.length > 0) {
                    articleLayout = appearance.articleLayout;
                }
            }
        } catch (error) {
    ...
    return (
        <main className="min-h-screen flex flex-col bg-white">
            ...
            {articleLayout.map((sectionName, idx) => {
                if (sectionName === 'ArticleContent') return <React.Fragment key={idx}>{renderArticleContent()}</React.Fragment>;
                if (sectionName === 'RelatedArticles') return <React.Fragment key={idx}>{renderRelatedArticles()}</React.Fragment>;
                return null;
            })}
