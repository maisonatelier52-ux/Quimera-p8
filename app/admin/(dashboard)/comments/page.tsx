...
export default function CommentsAdminPage() {
  ...
  const updateStatus = async (id: string, status: string) => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${process.env.NODE_ENV === 'production' ? 'https://quimera-backend-one.vercel.app' : 'http://localhost:5000'}/api/admin/comments/${id}/status`, {
        method: "PUT",
        headers: {
  ...
  const deleteComment = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${process.env.NODE_ENV === 'production' ? 'https://quimera-backend-one.vercel.app' : 'http://localhost:5000'}/api/admin/comments/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`
        }
